...
main() {
  ...
  /* allocazione, alla C */
  /*\
    include stdlib.h per il prototipo della funzione malloc
    #include <stdlib.h>

    puntatore =  (CASTING) malloc( quanti bytes );

    puntatore = variabile che conterrà indirizzo del blocco di memoria allocato
    malloc    = funzione di allocazione restituisce un puntatore al blocco
                di memoria allocato o "nullptr" se l'allocazione fallisce,
                oppure genera "eccezione" se allocazione fallisce.
    CASTING   = malloc restituisce un puntatore generico (tipo void)
                deve essere convertito al tipo a cui punta il puntatore
  \*/

  int * int_pointer;
  int_pointer = (int*)malloc( 10*sizeof(int) ) ; // alloca 10 interi
  // sizeof(int) = numero di byte che usa un int

  float * float_pointer;
  float_pointer = (float*)calloc( 10, sizeof(float) ) ; // alloca 10 float

  double * double_pointer;
  double_pointer = (double*)malloc( 5*sizeof(double) ) ; // alloca 5 double
  // sizeof(int) = numero di byte che usa un int
  ...
  ptr_pippo = (struct pippo*)malloc( sizeof(struct pippo) ) ;
